...
| **Row Calculator** | Set rules: rows alternate from a configurable front-row parity and always go **up** — 21, 22, 21, 22 — never down. Every row sits within a couple of people of the others so the group reads as one consistent block at a glance, and the final remainder joins the last two rows rather than leaving a stubby trailing row. Anyone still over stands at the side as an "extra", marked in amber and included in that row's queue. |
| **Height Zones / Queues** | One queue per row, numbered 1 (shortest) to N (tallest). Set rule — front-to-back the groups run **2, 1, 3, 4, 5 …**: the second-shortest takes the front row and the shortest stands behind them in row 2, so nobody is hidden; from row 3 back it ascends to the tallest. Each queue holds exactly its row's student count, so a queue empties into its row and the row comes out full. |
| **Teacher Placement** | Set rule: teachers always take the front row; VIP teachers take precedence — the contiguous centre-most seats (VIP 1 dead centre) and first in the call sequence. Overflow spills to Row 2, Row 3, … always as a centred block with students only at the two sides. |
